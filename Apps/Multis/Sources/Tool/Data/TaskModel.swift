...
public struct TaskModel: Identifiable, Comparable {
    ...
    /// 카테고리가 없는 항목들은 맨 아래로
    public static func sortedByCategory(_ tasks: [TaskModel]) -> [TaskModel] {
    ...
    public static func sortedByCategoryAndTime(_ tasks: [TaskModel]) -> [TaskModel] {
        return tasks.sorted { lhs, rhs in
            let lhsCategory = lhs.category?.title ?? "zzz_no_category"
            let rhsCategory = rhs.category?.title ?? "zzz_no_category"
    ...
    public static func groupedByCategory(_ tasks: [TaskModel]) -> [String: [TaskModel]] {
