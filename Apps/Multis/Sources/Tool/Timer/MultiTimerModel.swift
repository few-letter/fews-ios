...
public class MultiTimerModel: TimerModel {
  ...
  @ObservationIgnored
  @Dependency(\.goalClient)
  private var goalClient
  @ObservationIgnored
  @Dependency(\.taskClient)
  private var taskClient
  @ObservationIgnored
  ...
  public func fetch() {
    ...
    let goals: [TimeDocument] = goalClient.fetches().flatMap { item in
      item.times.map { key, _ in
        .init(item: .goal(item), date: key)
      }
    }
    documents = IdentifiedArrayOf(uniqueElements: tasks + goals)
  }
  ...
  public func add(document: TimeDocument, date: Date?) {
    ...
    case let .task(item):
      ...
      documents.append(.init(item: .task(new)))
    case let .goal(item):
      return
    }
  }
  ...
  public func update(document: TimeDocument) {
    ...
    case let .task(item):
      let new = taskClient.createOrUpdate(task: item)
      documents[id: document.id] = .init(item: .task(new))
    case let .goal(item):
      let new = goalClient.createOrUpdate(goal: item)
      documents[id: document.id] = .init(item: <#T##TimeDocumentIem#>, date: <#T##Date?#>).goal(new)
    }
  }
  ...
  private func updateTime(for document: TimeDocument, increment: Int) {
    ...
    case let .task(task):
      if let updatedTask = documents[id: task.id] {
        if case var .task(taskData) = updatedTask {
    ...
    case let .goal(goal):
      if let updatedGoal = documents[id: goal.id] {
        if case var .goal(goalData) = updatedGoal {
  ...
  private func saveEntity(for document: TimeDocument) {
    ...
    case let .task(task):
      if let currentDoc = documents[id: task.id] {
        if case let .task(taskData) = currentDoc {
    ...
    case let .goal(goal):
      if let currentDoc = documents[id: goal.id] {
        if case let .goal(goalData) = currentDoc {
