...
def run_appstore_preview():
    """앱스토어 프리뷰 이미지를 생성합니다."""
    appstore_dir = Path(__file__).parent / "appstore"
    run_script = appstore_dir / "run_appstore_preview.py"
    
    if not run_script.exists():
        print(f"❌ 앱스토어 프리뷰 스크립트를 찾을 수 없습니다: {run_script}")
        return False
    ...
    try:
        print("🎨 앱스토어 프리뷰 이미지 생성을 시작합니다...")
        print(f"📁 작업 디렉터리: {appstore_dir}")
        
        # 작업 디렉터리를 appstore로 변경하여 실행
        result = subprocess.run(
            [str(python_path), str(run_script)],
            cwd=str(appstore_dir),
            check=True
        )
        
        print("✅ 앱스토어 프리뷰 이미지 생성이 완료되었습니다!")
        return True
        
    except subprocess.CalledProcessError as e:
        print(f"❌ 앱스토어 프리뷰 생성 실패: {e}")
        return False
    except Exception as e:
        print(f"❌ 예상치 못한 오류: {e}")
        return False
...
def main():
    ...
    while True:
        ...
        if choice == 1:
            ...
            # 앱스토어 프리뷰 실행
            if run_appstore_preview():
                output_dir = Path(__file__).parent / "appstore" / "output"
