...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        while app_index < 0 or app_index >= len(available_apps):
            try:
                choice = input(f"Enter the number of the app to generate previews for (1-{len(available_apps)}): ")
    ...
    for device_type in supported_devices:
        for lang in supported_languages:
            print(f"\n--- Generating previews for {final_app_name} ({device_type}) in {lang} ---")
            generate_appstore_previews(app_name=final_app_name, language=lang, device_type=device_type)
